...
static void get_timeout(struct timespec *ts, int milliseconds) {
    // Ensure clock_gettime is successful, though errors are rare for CLOCK_REALTIME
    if (clock_gettime(CLOCK_REALTIME, ts) == -1) {
        // Handle error, e.g., by not modifying ts or setting a specific error state
        // For simplicity here, we'll proceed, but in robust code, this should be handled.
        perror("clock_gettime");
        // Potentially, don't add to ts if clock_gettime failed, or return an error.
    }
    
    // Add milliseconds to the current time
    // Use long long for nanosecond calculations to prevent overflow before normalization
    long long ns_to_add = (long long)milliseconds * 1000000LL;
    
    ts->tv_nsec += ns_to_add;
    
    // Normalize timespec
    // Add full seconds from nanoseconds to tv_sec
    ts->tv_sec += ts->tv_nsec / 1000000000LL;
    // Remainder nanoseconds
    ts->tv_nsec %= 1000000000LL;

    // Ensure tv_nsec is not negative if original ts->tv_nsec + ns_to_add was negative
    // (unlikely with positive milliseconds, but good for general timespec arithmetic)
    if (ts->tv_nsec < 0) {
        ts->tv_nsec += 1000000000LL;
        ts->tv_sec -= 1;
    }
